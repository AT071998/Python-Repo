...
def main():
    ...
    W_New = Weight_Updation(W,beta,Y,J)
    ...
    print("Updated W is, W = ")
    W_New1 = Weight_Updation(W_New, beta, Y, J1)
    display_Matrix(W_New1)
    print("=========================================")
...
def Calculate(X, Y, V, W):
    D1 = round((((X[0] - V[0][0]) ** 2) + (X[1] - V[1][0]) ** 2 + + (X[2] - V[2][0]) ** 2 +  (X[3] - V[3][0]) ** 2 + + (Y[0] -W[0][0]) ** 2 + + (Y[1] -W[1][0])**2),3)
    print("D1 = ",D1)
    D2 = round((((X[0] - V[0][1]) ** 2) + (X[1] - V[1][1]) ** 2 + + (X[2] - V[2][1]) ** 2 + (X[3] - V[3][1]) ** 2 + + (Y[0] -W[0][1]) ** 2 + + (Y[1] -W[1][1]) ** 2),3)
    print("D2 = ",D2)
    if D1 < D2:
        J = 1
    elif D2 < D1:
...
def Weight_Updation(W,beta,Y,J):
    for i in range(0,2):
        if J == 1:
           W[i][0] = round(W[i][0] + (beta*(Y[i]-W[i][0])),3)
        else:
           W[i][1] = round(W[i][1] + (beta *(Y[i]-W[i][1])),3)
    return  W
